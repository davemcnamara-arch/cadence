-- Update grade_song function to include teacher_reviewed field
...
BEGIN
  ...
  -- Add rating with teacher_reviewed = FALSE (new ratings need review)
  ...
  VALUES (
    ...
    p_notes,
    FALSE  -- New ratings need teacher review
  )
  ON CONFLICT (song_id, instrument_id, user_id)
  DO UPDATE SET
    assessed_level = EXCLUDED.assessed_level,
    checklist_responses_json = EXCLUDED.checklist_responses_json,
    notes = EXCLUDED.notes,
    teacher_reviewed = FALSE,  -- Mark as needing review again if updated
    date_graded = NOW()
